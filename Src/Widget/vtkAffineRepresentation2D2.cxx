/*=========================================================================
  ...
  Module:    vtkAffineRepresentation2D2.cxx
...
class vtkAffineRepresentation2D2Callback : public vtkCommand
{
public:
	static vtkAffineRepresentation2D2Callback* New() { return new vtkAffineRepresentation2D2Callback; }
	vtkTypeMacro(vtkAffineRepresentation2D2Callback, vtkCommand);
	...
	vtkAffineRepresentation2D2* Self = nullptr;
};

vtkStandardNewMacro(vtkAffineRepresentation2D2);

vtkCxxSetObjectMacro(vtkAffineRepresentation2D2,Property,vtkProperty2D);
vtkCxxSetObjectMacro(vtkAffineRepresentation2D2,SelectedProperty,vtkProperty2D);
vtkCxxSetObjectMacro(vtkAffineRepresentation2D2,TextProperty,vtkTextProperty);

#define VTK_CIRCLE_RESOLUTION 64

//----------------------------------------------------------------------
vtkAffineRepresentation2D2::vtkAffineRepresentation2D2()
{
  ...
  this->AffineRepresentation2D2Callback = vtkAffineRepresentation2D2Callback::New();
  this->AffineRepresentation2D2Callback->Self = this;
  this->TotalMatrix = vtkMatrix4x4::New();
...
vtkAffineRepresentation2D2::~vtkAffineRepresentation2D2()
{
  ...
  this->TotalMatrix->RemoveObserver(this->AffineRepresentation2D2Callback);
  this->TotalMatrix->Delete();
  this->AffineRepresentation2D2Callback->Delete();
  this->TempTransform->Delete();
}

//-------------------------------------------------------------------------
void vtkAffineRepresentation2D2::GetTransform(vtkTransform *t)
{
  this->CurrentTransform->Identity();
  //this->CurrentTransform->Translate(this->Origin[0],this->Origin[1],this->Origin[2]);
  this->CurrentTransform->Translate(
	  this->TotalMatrix->GetElement(0, 3), 
	  this->TotalMatrix->GetElement(1, 3), 
	  this->TotalMatrix->GetElement(2, 3));
  ...
  this->ApplyShear();
  switch (static_cast<ORIENTATION>(this->Orientation))
  {
  case ORIENTATION_XZ:
	  this->CurrentTransform->RotateY(vtkMath::DegreesFromRadians(this->CurrentAngle));
	  break;
  case ORIENTATION_YZ:
	  this->CurrentTransform->RotateX(vtkMath::DegreesFromRadians(this->CurrentAngle));
	  break;
  case ORIENTATION_XY:
  ...
  t->Concatenate(this->TotalMatrix);
}

void vtkAffineRepresentation2D2::SetUserMatrix(vtkMatrix4x4 * matrix)
{
	if (this->UserMatrix == matrix) {
		return;
	}
	if (this->UserMatrix) {
		//this->RemoveObserver(this->AffineRepresentation2D2Callback);
		this->UserMatrix->UnRegister(this);
	}
	this->UserMatrix = matrix;
	if (this->UserMatrix) {
		this->UserMatrix->Register(this);
		//this->UserMatrix->AddObserver(vtkCommand::ModifiedEvent, this->AffineRepresentation2D2Callback);
		  // for stoping the pipeline trick
		this->TotalMatrix->DeepCopy(this->UserMatrix);
		//this->Origin[0] = this->UserMatrix->GetElement(0, 3);
		//this->Origin[1] = this->UserMatrix->GetElement(1, 3);
		//this->Origin[2] = this->UserMatrix->GetElement(2, 3);
	}
}

//-------------------------------------------------------------------------
void vtkAffineRepresentation2D2::PlaceWidget(double bounds[6])
{
	vtkMatrix4x4* matrix = vtkMatrix4x4::New();
	
  matrix->SetElement(0, 3, (bounds[1] + bounds[0]) / 2.0);
  matrix->SetElement(1, 3, (bounds[3] + bounds[2]) / 2.0);
  matrix->SetElement(2, 3, (bounds[5] + bounds[4]) / 2.0);
  this->SetUserMatrix(matrix);
  //this->TotalTransform->Identity();
}

//-------------------------------------------------------------------------
//void vtkAffineRepresentation2D2::SetOrigin(double ox, double oy, double oz)
//{
...
//-------------------------------------------------------------------------
int vtkAffineRepresentation2D2::ComputeInteractionState(int X, int Y, int modify)
{
...
void vtkAffineRepresentation2D2::StartWidgetInteraction(double startEventPos[2])
{
...
// of the widget.
void vtkAffineRepresentation2D2::WidgetInteraction(double eventPos[2])
{
  ...
  //this->TempTransform->Concatenate(this->TotalMatrix);
  this->UserMatrix->DeepCopy(this->TempTransform->GetMatrix());
  this->Modified();
}

//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::EndWidgetInteraction(double vtkNotUsed(eventPos) [2])
{
  ...
  this->UserMatrix->DeepCopy(this->TempTransform->GetMatrix());
  this->TotalMatrix->DeepCopy(this->TempTransform->GetMatrix());
...
void vtkAffineRepresentation2D2::Translate(double eventPos[2])
{
...
//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::Scale(double eventPos[2])
{
...
//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::Rotate(double eventPos[2])
{
  double deltaAngle;
...
void vtkAffineRepresentation2D2::ApplyShear()
{
}


//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::Shear(double eventPos[2])
{
  // Determine the relative motion
  double d[3];
  d[0] = eventPos[0] - this->StartEventPosition[0];
...
//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::Highlight(int highlight)
{
...
//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::CreateDefaultProperties()
{
...
//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::UpdateText(const char *text, double eventPos[2])
{
...
//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::BuildRepresentation()
{
        ...
        this->Renderer->GetVTKWindow()->GetMTime() > this->BuildTime) )
  {
    // Determine where the origin is on the display
	  // this is the situation that the UserMatrix is changed externally
	...
	// Must call for update its actor
	...
	if (this->UserMatrix && 
		!this->UserMatrix->HasObserver(vtkCommand::ModifiedEvent, 
		this->AffineRepresentation2D2Callback)) {
...
//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::ShallowCopy(vtkProp *prop)
{
  vtkAffineRepresentation2D2 *rep =
    vtkAffineRepresentation2D2::SafeDownCast(prop);
  if ( rep )
  {
...
//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::GetActors2D(vtkPropCollection *pc)
{
...
//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::ReleaseGraphicsResources(vtkWindow *win)
{
...
//----------------------------------------------------------------------
int vtkAffineRepresentation2D2::RenderOverlay(vtkViewport *viewport)
{
  this->BuildRepresentation();
...
//----------------------------------------------------------------------
void vtkAffineRepresentation2D2::PrintSelf(ostream& os, vtkIndent indent)
{
